...
a frozen spiking simulation** captioned as though the neurons were taught
something, and some of it is scripted. Building that is worth doing. The caption
is the problem.
...
| **Hand type decodable from Kenyon cells** | **1.000** (grouped CV 0.995) | chance 0.117; driven-ORN-count-only confound 0.267. The old encoding's KC probe, on an *easier* binary label, managed 0.598 on a different task, so it is not a controlled comparison. |
| **…at the descending neurons** | **0.842** | but the DN *intensity* probe is 0.918, so DNs still carry how much more cleanly than what |
...
* **"Real wiring beats shuffled" is retracted.** Under the glomerular encoding a
  ...
  on and leaves a near-silent network, so it compares a working brain against a
...
0.605 against a 0.593 majority floor, while the *same* DN spikes decoded "how
...
with respect to glomerular identity. A near-null here **agrees with the
biology**, and a large effect would have been the surprise.
